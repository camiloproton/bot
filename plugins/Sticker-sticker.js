...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    try {
        ...
        if (/webp|image|video/g.test(mime)) {
            ...
            let out
            try {
                stiker = await sticker(img, false, '❤̅️̅ ', 'wa.me/542214186048')
            } catch (e) {
                console.error(e)
            } finally {
                if (!stiker) {
                    ...
                    stiker = await sticker(false, out, '❤̅️̅ ', 'wa.me/542214186048')
                }
            }
        } else if (args[0]) {
            if (isUrl(args[0])) stiker = await sticker(false, args[0], '❤̅️̅ ', 'wa.me/542214186048')
